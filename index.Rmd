...
birth_reg_county <- birth_reg_county %>%
  mutate(cat=ifelse(Proportion<31, "<31", ifelse(Proportion>30 & Proportion<43, "31-42", ifelse(Proportion>42 & Proportion <52, "43-51",
                                                                                                ifelse(Proportion>51&Proportion<73, "52-72", ">73")))))
...
school_att_county <- school_att_county %>%
  mutate(cat=ifelse(Proportion<52, "<52", ifelse(Proportion>51 & Proportion<76, "51-75", ifelse(Proportion>75 & Proportion <90, "75-89",
                                                                                                ifelse(Proportion>89&Proportion<104, "90-103", ">103")))))
...
RTA_county <- RTA_county %>%
  mutate(cat=ifelse(Proportion<33, "<33", ifelse(Proportion>32 & Proportion<85, "33-84", ifelse(Proportion>84 & Proportion <211, "85-210",
                                                                                                ifelse(Proportion>210 &Proportion<1062, "211-1061", ">1062")))))
...
covid_county3 <- covid_county3 %>%
  mutate(cat=ifelse(Proportion<9.6, "<9.6", ifelse(Proportion>9.5 & Proportion<18.7, "9.6-18.6", ifelse(Proportion>18.6 & Proportion <30.1, "18.7-30.0",
                                                                                                ifelse(Proportion>30.0&Proportion<48.0, "30.1-47.9", ">48.0")))))
...
cash_transfer_county1 <- cash_transfer_county1 %>%
  mutate(cat=ifelse(Proportion<0.7, "<0.7", ifelse(Proportion>0.6 & Proportion<2.6, "0.7-2.5", ifelse(Proportion>2.5 & Proportion <8.1, "2.6-8.0",
                                                                                                ifelse(Proportion>8.0&Proportion<20.0, "8.1-19.9", ">19.9")))))
...
background_characteristics_map <- background_characteristics_map %>%
  mutate(cat=ifelse(weighted_perc<0.8, "<0.8", ifelse(weighted_perc>0.7 & weighted_perc<1.3, "0.8-1.2", ifelse(weighted_perc>1.2 & weighted_perc <2.1, "1.3-2.0",
                                                                                                ifelse(weighted_perc>2.0&weighted_perc<2.9, "2.1-2.8", ">2.8")))))
...
occupation_county3 <- occupation_county3 %>%
  mutate(cat=ifelse(Proportion<1.9, "<1.9", ifelse(Proportion>1.8 & Proportion<5.5, "1.9-5.4", ifelse(Proportion>5.4 & Proportion <9.5, "5.5-9.4",
                                                                                                ifelse(Proportion>9.4&Proportion<18.5, "9.5-18.4", ">18.4")))))
...
water_collector_county1 <- water_collector_county1 %>%
  mutate(cat=ifelse(Proportion<3.1, "<3.1", ifelse(Proportion>3.0 & Proportion<7.1, "3.1-7.0", ifelse(Proportion>7.0 & Proportion <17.5, "7.1-17.4",
                                                                                                ifelse(Proportion>17.4&Proportion<62.0, "17.5-61.9", ">61.9")))))
...
water_treat_county1 <- water_treat_county1 %>%
  mutate(cat=ifelse(Proportion<0.3, "<0.3", ifelse(Proportion>0.2 & Proportion<3.0, "0.3-2.9", ifelse(Proportion>2.9 & Proportion <25.3, "3.0-25.2",
                                                                                                 ">25.2"))))
...
excreta_mgmnt_county <- excreta_mgmnt_county %>%
  mutate(cat=ifelse(Proportion<1, "<1.0", ifelse(Proportion>0.9 & Proportion<10.7, "0.9-10.6", ifelse(Proportion>10.6 & Proportion <47.8, "10.7-47.7",
                                                                                                 ">47.7"))))
...
child_stool_disp_county1 <- child_stool_disp_county1 %>%
  mutate(cat=ifelse(Proportion<2, "<2", ifelse(Proportion>1.9 & Proportion<10.4, "2.0-10.3", ifelse(Proportion>10.3 & Proportion <57.2, "10.4-57.1",
                                                                                                 ">57.1"))))
...
handwashing_county1 <- handwashing_county1 %>%
  mutate(cat=ifelse(Proportion<13.1, "<13.1", ifelse(Proportion>13.0 & Proportion<39.8, "13.1-39.7", ifelse(Proportion>39.7 & Proportion <63.8, "39.8-63.7",
                                                                                                 ">63.7"))))
...
marriage_data1 <- marriage_data1 %>%
  mutate(cat=ifelse(Value<16.1, "<16.1", ifelse(Value>16.0 & Value<20.1, "16.1-20.0", ifelse(Value>20.0 & Value <40.1, "20.1-40.0",
                                                                                                 ">40.0"))))
...
TFRbycounty.2022_1 <- TFRbycounty.2022_1 %>%
  mutate(cat=ifelse(Proportion<3.2, "<3.2", ifelse(Proportion>3.1 & Proportion<3.6, "3.2-3.5", ifelse(Proportion>3.5 & Proportion <4.4, "3.6-4.3",
                                                                                                 ">4.3"))))
...
median_age_county1 <- median_age_county1 %>%
  mutate(cat=ifelse(proportion<19.9, "<19.9", ifelse(proportion>19.9 & proportion<20.8, "19.9-20.7", ifelse(proportion>20.7 & proportion <21.2, "20.8-21.1",
                                                                                                 "21.2-22.3"))))
...
teenpregnancybycountyvisual1 <- teenpregnancybycountyvisual1 %>%
  mutate(cat=ifelse(Percent<1.5, "<1.5", ifelse(Percent>1.4 & Percent<5.4, "1.5-5.3", ifelse(Percent>5.3 & Percent <12.7, "5.4-12.6",
                                                                                                 "12.7-50.1"))))
...
renderGirafe({
  ...
  geom_sf_interactive(aes(fill=proportion_category, tooltip=paste0(Name,": ",Proportion,"%")),color="grey80", size=0.0)+
    theme_void()+
    facet_wrap(Category~Participants, nrow=2, labeller = label_wrap_gen(multi_line = 3))+
...
micronutrient <- fread("data/table_11/table11_12_micronutrient_by_background_xtics.csv") %>%
  mutate(Proportion=ifelse(Proportion<0.5, Proportion, round(Proportion)))
...
renderPlot({ggplot(house_land_ownsership[house_land_ownsership$`Background characteristics`%in%input$home_own,],aes(y = Category, x=value, fill = variable)) + 
  geom_col(  position = 'dodge')+facet_grid(~Gender)+
    geom_text(aes(label=paste0(round(value),"%")), position=position_dodge(width=1),vjust=-0.5)+
  scale_fill_brewer(palette = "Set1")+
  theme_bw()+
...
renderGirafe({if(input$agric_own!="County"){
  ...
  geom_col(position="dodge",fill="#abd9e9") + geom_text(aes(label=paste0(value,"%")), position=position_dodge(width=1), vjust=-0.3)+
...
renderGirafe({if(input$nonagric_own!="County"){
  ...
  geom_col(position="dodge", fill="#abd9e9") + geom_text(aes(label=paste0(value,"%")), position=position_dodge(width=1), hjust=-0.5)+
  facet_grid(Gender~variable, labeller = label_wrap_gen(width=10,multi_line = T)) + 
  theme_bw() +coord_flip()+
  labs( x = "", y="%", fill="")+theme(text=element_text(size=16, face="bold"))
...
renderGirafe({if(input$mobile!="County"){
  ...
  geom_col(position="dodge", fill="#abd9e9") + geom_text(aes(label=paste0(value,"%")), position=position_dodge(width=1), vjust=-0.5)+
...
renderGirafe({if(input$decision_making!="County"){
  ...
  geom_col(position="dodge", fill="cyan3") + geom_text(aes(label=paste0(value,"%")), position=position_dodge(width=1), vjust=-0.3)+
...
renderGirafe({if(input$safer_sex!="County"){
  ...
  print(att1x)}
    ...
    att2 <-ggplot(T15.10C ,aes(fill= abs(value))) +
   geom_sf_interactive(aes(tooltip=paste0(Name, ", ", value)))+
