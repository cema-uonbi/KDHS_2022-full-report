...
renderPlot({
      ...
      ggplot(fertility_rate[fertility_rate$characteristic%in%input$tfr1,],aes(x=category, y=proportion))+
...
renderPlot({
  ...
  ggplot(median_age%>%filter(characteristic%in%input$median_age1),aes(x=Variable, y=proportion))+
  geom_bar(position="dodge", stat="identity", fill="cyan3")+
  theme_bw()+
  geom_text(aes(label=paste0(proportion)))+
...
### **Median age at first live birth (By County)**
...
renderPlot({
  ...
  theme(text=element_text(size=18, face="bold"))+
  geom_text(aes(label = lab), hjust=0.5,vjust=0.2, size=4)+ylim(0,100)
...
### **Teenage pregnancy (By County)**

**Choose the category to display**

```{r}
selectInput("teenage_county", "",
...
renderGirafe({
  ...
  teenpreg_map <- ggplot(teenpregnancybycountyvisual1[teenpregnancybycountyvisual1$Pregnancy%in%input$teenage_county,])+
  geom_sf_interactive(aes(fill=proportion_category, tooltip=paste0(Name,": ",Percent)),color="grey80", size=0.0)+
    theme_void()+
    #scale_fill_brewer(palette = "YlOrBr")+  
    ...
    labs(x="", y="",fill="Percent (%)")+
    theme(text=element_text(size=14, face="bold")) 
  teenpreg_mapx<-girafe(ggobj = teenpreg_map, width_svg = 5, height_svg = 5,
                   options = list(
                     opts_hover(css = "fill:#666666;cursor:pointer;"),
                     opts_selection(css = "fill:orange;", type = "multiple"),
                     opts_sizing(rescale = T),
...
**Percent distribution of rhythm users, SDM users, and all women age 15-49 by knowledge of the fertile period during the ovulatory cycle**
...
renderPlot({ggplot(need_demand[need_demand$variable%in%input$need_demand,],aes(x=Characteristic, y=Proportion, fill=type))+
  ...
  theme_bw()+facet_grid(~Category)+
    geom_text(aes(label=paste0(round(Proportion))), position = position_dodge(0.5))+
  labs( y="Percent (%)", title="", x="", fill="")+
  scale_fill_brewer(palette ="Set1")+
  theme(text=element_text(size=16, face="bold"))}, width=1600, height=700 )
...
**Select the Source of message to display**

```{r}
selectInput("fp_messages_county", "",
              c("Friends/peers"="Friends/peers",
                "Internet"="Internet",
                "Mobile phone"="Mobile phone",
                "Newspaper/magazine"="Newspaper/magazine",
                "Outdoor sign/billboard"="Outdoor sign/billboard",
                "Poster/leaflet/brochure"="Poster/leaflet/brochure",
                "Radio"="Radio",
...
fp_messages_county <-fread("data/table_7/table_7_20C_fp_messages_total_county.csv")%>%
  mutate(County=recode(County,"Elgeyo/Marakwet"="Elgeyo Marakwet", "Murang’a"="Muranga", "Tharaka-Nithi"="Tharaka Nithi", "Nairobi City"="Nairobi", "Taita/Taveta"="Taita Taveta"))%>%
  mutate(proportion_category=ifelse(Proportion<20,"<20%", ifelse(Proportion>19.9&Proportion<41,"20%-40%", ifelse(Proportion>40.9&Proportion<61,"41%-60%", ifelse(Proportion>60.9&Proportion<81,"61%-80%",">80%")))))
...
renderGirafe({
  ...
  fp_mess_map2 <- ggplot(fp_messages_county1%>%filter(Category%in%input$fp_messages_county))+
  geom_sf_interactive(aes(fill=proportion_category, tooltip=paste0(Name,": ",Proportion,"%")),color="grey80", size=0.0)+
    theme_void()+
    facet_grid(~Participants)+
    ...
    labs(x="", y="",fill="Percent (%)")+
    theme(text=element_text(size=14)) 
  fp_mess_map2x<-girafe(ggobj = fp_mess_map2, width_svg = 5, height_svg = 5,
                   options = list(
                     opts_hover(css = "fill:#666666;cursor:pointer;"),
                     opts_selection(css = "fill:orange;", type = "multiple"),
                     opts_sizing(rescale = T),
...
renderPlot({ggplot(antenatal_care1%>%filter(`Background characteristics`%in%input$antenatal_care_visits),aes(x=Category, y=Proportion, fill=`Value`))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+scale_fill_brewer(palette = "Set1")+
  geom_text(aes(label=paste0(Proportion)), position = position_dodge(0.6))+
...
### **Number of antenatal care visits (By County)**
...
anc_number_county <-fread("data/table_9/table_9_2C_ancvisits_number_by_county.csv")%>%
  mutate(County=recode(County,"Elgeyo-Marakwet"="Elgeyo Marakwet", "Murang'a"="Muranga", "Tharaka-Nithi"="Tharaka Nithi", "Nairobi City"="Nairobi", "Taita/Taveta"="Taita Taveta"))%>%
  mutate(proportion_category=ifelse(Proportion<40,"<40%",">40%"))

anc_number_county$proportion_category <- as.factor(fct_relevel(anc_number_county$proportion_category, c( "<40%",">40%")))
...
renderGirafe({
  ...
  geom_sf_interactive(aes(fill=proportion_category, tooltip=paste0(Name,": ",Proportion,"%")),color="grey80", size=0.0)+
    ...
    labs(x="", y="",fill="Percent (%)")+
    theme(text=element_text(size=14)) 
  anc_number_map2x<-girafe(ggobj = anc_number_map2, width_svg = 5, height_svg = 5,
                   options = list(
                     opts_hover(css = "fill:#666666;cursor:pointer;"),
                     opts_selection(css = "fill:orange;", type = "multiple"),
                     opts_sizing(rescale = T),
...
renderGirafe({
  ...
  map2<- ggplot(hiv_testc1[hiv_testc1$Category%in%input$hiv_test,])+
  geom_sf_interactive(aes(fill=proportion_groups, tooltip=paste0(Name,": ",Proportion,"%")),color="grey80", size=0.0)+
  theme_void()+facet_wrap(~Participants)+
  scale_fill_brewer(palette = "YlGn")+
  #scale_fill_gradient(low = "#fee6ce", high = "#7f2704")+
  labs(x="", y="",fill="Percent (%)")+
  theme(text=element_text(size=14))
