...
renderGirafe({
  ...
  labs(y="Percent (%)", x="")+
    ...
    theme(strip.background =element_rect(fill="darkorange"))+
    geom_text(aes(label=paste0(Proportion, "%"))+ylim(0,100), position=position_dodge(0.6),vjust=0.5,hjust=-0.2, size=4, color="black")
...
renderPlot({if(input$median_age%in%c("2014", "2022")){
  ...
  theme(text=element_text(size=14))}
      ...
      ggplot(median_age[median_age$characteristic%in%input$median_age,],aes(x=Variable, y=proportion, fill=category))+
  geom_dotplot(binaxis='y', stackdir='center', dotsize  = 3)+
  theme_bw()+
  geom_text(aes(label=paste0(proportion)))+
...
renderPlot({
  ...
  geom_bar(position="dodge", stat="identity", fill="cyan3")+
  theme_bw()+coord_flip()+
  labs( y="Rate", title="", x="", fill="")+
    geom_text(aes(label=paste0(Wanted.fertility.rates)))+
  theme(text=element_text(size=18, face="bold"))
}, width=1200, height=800)
...
renderPlot({
  ggplot(early_childhood_mortality[early_childhood_mortality$Variable%in%input$early_childhood_mortality,],aes(x=Characteristic, y=proportion, fill=category))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+
...
### **Childhood mortality (By County:maps)**

```{r}
selectInput("early_childhood_mortality_county1", "",
...
early_childhood_mortality_county$proportion_category <- as.factor(fct_relevel(early_childhood_mortality_county$proportion_category, c( "<20", "20-40", ">40")))
...
renderGirafe({
  ...
  mort_map2 <- ggplot(early_childhood_mortality_county1[early_childhood_mortality_county1$`category`%in%input$early_childhood_mortality_county1,])+
  geom_sf_interactive(aes(fill=proportion_category, tooltip=paste0(Name,": ",proportion)),color="grey80", size=0.0)+
    ...
    theme(text=element_text(size=14)) 
  mort_map2x<-girafe(ggobj = mort_map2, width_svg = 5, height_svg = 5,
                   options = list(
                     opts_hover(css = "fill:#666666;cursor:pointer;"),
                     opts_selection(css = "fill:orange;", type = "multiple"),
...
renderPlot({
  ...
  ggplot(early_childhood_mortality_county[early_childhood_mortality_county$category%in%input$early_childhood_mortality_county2,], aes(x=reorder(county,proportion), y=proportion, fill=proportion_category))+
    geom_col()+coord_flip()+
    theme_bw()+ 
    scale_fill_brewer(palette = "YlOrBr")+
    #scale_fill_manual(values = c("#78c679","#ffffcc","#feb24c","#e31a1c"))+
...
perinatal_mortality <-fread("data/table_8/Table_8.4_Perinatal Mortality.csv") %>%
  ...
  mutate(variable=ifelse(characteristic%in%c("No education","Primary","Secondary","More than secondary"),"Education",variable))
...
renderPlot({
  ggplot(perinatal_mortality[perinatal_mortality$variable%in%input$perinatal_mortality,],aes(x=characteristic, y=proportion, fill=category))+
...
### **Perinatal mortality (By County):map**
...
perinatal_mortality_county$proportion_category <- as.factor(fct_relevel(perinatal_mortality_county$proportion_category, c( "<20", "20-40", ">40")))
...
renderPlot({
  ...
  ggplot(perinatal_mortality_county, aes(x=reorder(county,proportion), y=proportion, fill=proportion_category))+
    geom_col()+coord_flip()+
    theme_bw()+ 
    scale_fill_brewer(palette = "YlOrBr")+
    #scale_fill_manual(values = c("#78c679","#ffffcc","#feb24c","#e31a1c"))+
...
### **Children Nutritional Status (By County:maps)**
...
renderGirafe({if(input$nutr2%in%"HFA"){
  ...
  nut_map2x<-girafe(ggobj = nut_map2, width_svg = 5, height_svg = 5,
                   options = list(
                     ...
                     opts_zoom(max = 4)))
  print(nut_map2x)}
  else if(input$nutr2%in%"WFA"){
    nutc1 <- nutc1%>%
    mutate(threshold=ifelse(perc_below2sd>2.4&perc_below2sd<5,"Low (2.5%- <5%)", ifelse(perc_below2sd>4.9&perc_below2sd<10,"Medium (5%- <10%)", ifelse(perc_below2sd>9.9&perc_below2sd<15,"High (10%- <15%)","Very High (>=15%)"))))
  ...
  nut_map2 <- ggplot(nutc1[nutc1$indicator%in%input$nutr2,], aes(fill=threshold))+
    ggiraph::geom_sf_interactive(aes(tooltip = paste0(County,": ", perc_below2sd,"%")),size=0.3)+
    theme_void()+
...
### **Micronutrient supplementation & deworming (By County)**

```{r}
selectInput("micronutrients_county", "",
              c("Deworming (last 6 months)"="Deworming last 6 months",
                "Iron supplements (last 6 months)"="Iron supplements last 6 months",
                "Iron tablets/syrups (last 12 months)"="Iron tablets/syrups last 12 months",
                "Micronutrient powders (last 12 months)"="Micronutrient powders last 12 months",
                "Vitamin A (last 6 months)"="Vit. A last 6 months"))
```
...
micronutrient_county1 <- full_join(county, micronutrient_county, by=c("Name"="County"))%>%
  mutate(proportion_category=ifelse(Proportion<50,"<50%", ifelse(Proportion>49&Proportion<81,"50-80%",">80%")))
...
renderGirafe({
  ...
  map1<- ggplot(micronutrient_county1[micronutrient_county1$`Category`%in%input$micronutrients_county,])+
  geom_sf_interactive(aes(fill=proportion_category, tooltip=paste0(Name,": ",Proportion,"%")),color="grey80", size=0.0)+
  theme_void()+
...
renderPlot({
  ggplot(nutrition_women[nutrition_women$`Background characteristics`%in%input$nutrition_women,],aes(x=Value, y=Proportion, fill=Category))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+
...
### **Women nutritional status (By County)**
...
### **Women minimum dietary diversity (By County)**

```{r}
selectInput("mdd_county", "",
              c("MDD-W"="MDD-W",
                "Sweet Beverage Consumption"="Sweet Beverage Consumption",
                "Unhealthy food consumption"="Unhealthy food consumption"))
```
...
mdd_county1 <- full_join(county, mdd_county, by=c("Name"="County"))%>%
  mutate(proportion_category=ifelse(Proportion<15,"<15%", ifelse(Proportion>14&Proportion<31,"15%-30%",">30%")))
mdd_county1$proportion_category<-as.factor(fct_relevel(mdd_county1$proportion_category, c("<15%", "15%-30%", ">30%")))

```

```{r}

renderGirafe({if(input$mdd_county%in%"MDD-W"){

  map1<- ggplot(mdd_county1%>%filter(Category%in%"MDD-W"))+
  geom_sf_interactive(aes(fill=proportion_category, tooltip=paste0(Name,": ",Proportion,"%")),color="grey80", size=0.0)+
  theme_void()+
  scale_fill_brewer(palette = "YlGn")+  
  #scale_fill_manual(values = c("#e31a1c","#ffffcc","#78c679"))+
...
else if(input$mdd_county%in%"Sweet Beverage Consumption"){
  map1<- ggplot(mdd_county1%>%filter(Category%in%"Sweet Beverage Consumption"))+
  geom_sf_interactive(aes(fill=proportion_category, tooltip=paste0(Name,": ",Proportion,"%")),color="grey80", size=0.0)+
  theme_void()+
  ...
  print(x)
    ...
    map1<- ggplot(mdd_county1%>%filter(Category%in%input$mdd_county))+
  geom_sf_interactive(aes(fill=proportion_category, tooltip=paste0(Name,": ",Proportion,"%")),color="grey80", size=0.0)+
  theme_void()+
  scale_fill_brewer(palette = "OrRd")+  
  #scale_fill_manual(values = c("#e31a1c","#ffffcc","#78c679"))+
...
renderPlot({
  ggplot(iron_women[iron_women$`Background characteristics`%in%input$iron_women,],aes(x=Value, y=Proportion, fill=Category))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+
...
### **Iron-containing supplementation during pregnancy (By County)**
...
renderPlot({if(input$itn!="County"){ggplot(itn_possess[itn_possess$`Background characteristics`%in%input$itn,],aes(x=Value, y=Proportion, fill=Category))+
  ...
  theme(text=element_text(size=16, face="bold"))+
  ylim(0,100)}
  else{}}, width=1600, height=700 )
```

### **Household possession of mosquito nets (By county)**
...
itn_possess_county1$proportion_category <- as.factor(fct_relevel(itn_possess_county1$proportion_category, c("<30%", "30%-60%", ">60%")))
...
renderPlot({
 if(input$itn_source!="County"){
  ...
  theme_bw()+coord_flip()+
      geom_text(aes(label=paste0(Proportion,"%")), position = position_dodge(0.6))+
  labs( y="PPercent (%)", title="", x="Source of ITN", fill="")+
  scale_fill_brewer(palette = "Set1")+
  geom_text(aes(label=paste0(Proportion)), position = position_dodge(0.7))+
  theme(text=element_text(size=16, face="bold"))+
  ylim(0,100)
...
### **Source of mosquito nets (By county)**
...
itn_source_county1$proportion_category <- as.factor(fct_relevel(itn_source_county1$proportion_category, c("<30%", "30%-60%", ">60%")))
...
renderPlot({if(input$itn_access%in%c("Residence", "Endemicity zone","Wealth quintile")){ggplot(itn_access[itn_access$`Background characteristics`%in%input$itn_access,],aes(x=Value, y=Proportion))+
...
itn_access_county1$proportion_category <- as.factor(fct_relevel(itn_access_county1$proportion_category, c("<30%", "30%-60%", ">60%")))
...
renderPlot({if(input$itn_use%in%c("Residence", "Endemicity zone","Wealth quintile")){ggplot(itn_use[itn_use$`Background characteristics`%in%input$itn_use,],aes(x=Value, y=Proportion, fill=Category))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+
  labs( y="Percent (%)", title="", x="", fill="Household ITN ownership")+
  scale_fill_brewer(palette = "Set1")+
...
### **Use of mosquito nets by households (By County)**
...
itn_use_county1$proportion_category <- as.factor(fct_relevel(itn_use_county1$proportion_category, c("<30%", "30%-60%", ">60%")))
...
renderPlot({if(input$itn_itn%in%c("Residence", "Endemicity zone","Wealth quintile")){ggplot(existing_itn_use[existing_itn_use$`Background characteristics`%in%input$itn_itn,],aes(x=Value, y=Proportion))+
  geom_bar(position="dodge", stat="identity", fill="#74add1")+
  theme_bw()+
  labs( y="Percent (%)", title="", x="", fill="")+
  scale_fill_brewer(palette = "Set1")+
...
### **Use of existing ITNs (By County)**
...
existing_itn_use_county1$proportion_category <- as.factor(fct_relevel(existing_itn_use_county1$proportion_category, c("<30%", "30%-60%", ">60%")))
...
renderPlot({if(input$nets_children%in%c("Residence", "Endemicity zone","Wealth quintile")){ggplot(children_itn_use[children_itn_use$`Background characteristics`%in%input$nets_children,],aes(x=Value, y=Proportion,fill=Category))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+
  labs( y="Percent (%)", title="", x="", fill="")+
  scale_fill_brewer(palette = "Set1")+
...
### **Use of mosquito nets:children (By County)**
...
children_itn_use_county1$proportion_category <- as.factor(fct_relevel(children_itn_use_county1$proportion_category, c("<30%", "30%-60%", ">60%")))
...
renderPlot({if(input$nets_notuse%in%c("Residence", "Endemicity zone","Wealth quintile")){ggplot(reason_itn_not_used[reason_itn_not_used$`Background characteristics`%in%input$nets_notuse,],aes(x=reorder(Reason,Proportion), y=Proportion,fill=Value))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+
  ...
  ylim(0,100)+
  coord_flip()}
  else{ggplot(reason_itn_not_used_total,aes(x=reorder(Reason,Proportion), y=Proportion))+
  geom_bar(fill="#74add1",position="dodge", stat="identity")+
  theme_bw()+
  labs( y="Percent (%)", title="", x="Reason", fill="")+
  #scale_fill_brewer(palette = "Set1")+
...
### **Mosquito net not used previous night (By county)**
...
reason_itn_not_used_county1$proportion_category <- as.factor(fct_relevel(reason_itn_not_used_county1$proportion_category, c("<30%", "30%-60%", ">60%")))
...
renderPlot({if(input$ipt_women%in%c("Residence", "Endemicity zone","Wealth quintile")){ggplot(iptp[iptp$`Background characteristics`%in%input$ipt_women,],aes(x=Value, y=Proportion,fill=Category))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+
...
### **Use of Intermittent Preventive Treatment by women during pregnancy (By County)**
...
renderPlot({if(input$mal_fever%in%c("Residence", "Endemicity zone","Mother's education","Wealth quintile")){ggplot(mal_fever[mal_fever$`Background characteristics`%in%input$mal_fever,],aes(x=Value, y=Proportion,fill=Category))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+
  labs( y="Percent (%)", title="", x="", fill="")+
  scale_fill_brewer(palette = "Set1")+
...
### **Children with fever: careseeking, prompt treatment & diagnosis (By County)**
...
mal_fever_county1$proportion_category <- as.factor(fct_relevel(mal_fever_county1$proportion_category, c("<30%", "30%-60%", ">60%")))
...
renderPlot({if(input$hiv_kap%in%c("Age", "Education","Marital status", "Residence")){
  ...
  geom_text(aes(label=paste0(Proportion)), position=position_dodge(0.6))+ylim(0,100)
}
 else{
 } 
}, width=1600, height=700)
...
hiv_testc1$cats <- fct_relevel(hiv_testc1$cats, "<40%", "40-80%", ">80%")
...
renderPlot({if(input$bp%in%c("Age", "Education", "Residence", "Wealth quintile")){ggplot(bp[bp$`Background characteristics`%in%input$bp,],aes(x=Value, y=Proportion,fill=Category))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+
...
### **High Blood Pressure/Hypertension (By County)**
...
bp_county1$proportion_category <- as.factor(fct_relevel(bp_county1$proportion_category, c("<10%", "10%-20%", ">20%")))
```
...
renderPlot({if(input$depression%in%c("Age","Residence", "Education","Wealth quintile")){ggplot(depression[depression$`Background characteristics`%in%input$depression,],aes(x=Value, y=Proportion,fill=Category))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+
  facet_wrap(.~Participants)+
  labs( y="Percent (%)", title="", x="", fill="")+
...
### **Depression and/or Anxiety (By County)**
...
depression_county1$proportion_category <- as.factor(fct_relevel(depression_county1$proportion_category, c("<10%", "10%-20%", ">20%")))
```
...
renderPlot({if(input$arthritis%in%c("Age","Residence", "Education","Wealth quintile")){ggplot(arthritis[arthritis$`Background characteristics`%in%input$arthritis,],aes(x=Value, y=Proportion,fill=Category))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+
  facet_wrap(.~Participants)+
...
renderPlot({if(input$physical_activity%in%c("Age","Residence", "Education","Wealth quintile")){ggplot(physical_activity[physical_activity$`Background characteristics`%in%input$physical_activity,],aes(x=Value, y=`Activity time`))+
  geom_bar(position="dodge", stat="identity", fill="#74add1")+
  theme_bw()+
  facet_wrap(.~Participants)+
  labs( y="Physical Activity Time (in minutes)", title="", x="", fill="")+
  scale_fill_brewer(palette = "Set1")+
  geom_text(aes(label=paste0(`Activity time`)), position=position_dodge(0.7), vjust=0.2)+
  theme(text=element_text(size=16, face="bold"))+
  ylim(0,1000)}
  else{
  }}, width=1600, height=700 )
...
renderPlot({if(input$breast_cancer%in%c("Age","Residence", "Education","Wealth quintile")){ggplot(breast_cancer[breast_cancer$`Background characteristics`%in%input$breast_cancer,],aes(x=Value, y=Proportion,fill=Category))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+
  labs( y="Percent (%)", title="", x="", fill="")+
  scale_fill_brewer(palette = "Set1")+
...
renderPlot({if(input$cervical_cancer%in%c("Age","Residence", "Education","Wealth quintile")){ggplot(cervical_cancer[cervical_cancer$`Background characteristics`%in%input$cervical_cancer,],aes(x=Value, y=Proportion,fill=Category))+
  geom_bar(position="dodge", stat="identity")+
  theme_bw()+
  labs( y="Percent (%)", title="", x="", fill="")+
  scale_fill_brewer(palette = "Set1")+
