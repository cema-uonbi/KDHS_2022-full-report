...
insurance <- insurance[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
renderPlot({
    ggplot(housing[housing$Characteristics%in%input$housing], aes(x=Value, y=Proportion, fill=Category))+geom_bar(stat = 'identity', position = position_dodge())+#,aes(tooltip=paste0(Coverage, "%"))) +
  labs(y="Percent (%)", x="", fill="")+ylim(0,100)+
...
renderPlot({
    ggplot(cooking[cooking$Characteristics%in%input$cooking], aes(x=Value, y=Proportion, fill=Category))+geom_bar(stat = 'identity', position = position_dodge())+#,aes(tooltip=paste0(Coverage, "%"))) +
  labs(y="Percent (%)", x="", fill="")+ylim(0,100)+
    scale_fill_brewer(palette = "Set1")+coord_flip()+
    theme_bw()+theme(text=element_text(size=16, face = "bold")) +
    theme(strip.background =element_rect(fill="darkorange"))+
...
hh_possession <-fread("data/table_2/table2_5_HH_possession.csv") |> as.data.table() |>
      (\(x) {x$Proportion <- fifelse(x$Proportion > 0.5,  round(x$Proportion), x$Proportion); x})()
...
birth_reg_county <- fread("data/table_2/table2_10C_Birth_registration_by_county.csv")
birth_reg_county[, Proportion := as.numeric(Proportion)]

breaks<- classIntervals(birth_reg_county$Proportion, n=5, style="quantile", na.rm=T)

# birth_reg_county <- birth_reg_county |>
...
birth_reg_county <- birth_reg_county[, cat := fifelse(Proportion < 31, "<31",
fifelse(Proportion > 30 & Proportion < 43, "31-42",
fifelse(Proportion > 42 & Proportion < 52, "43-51",
...
birth_reg <- fread("data/table_2/table2_10_Birth_registration_by_background_xtics.csv") |>
  (\(x) {x$Proportion <- fifelse(x$Proportion > 0.5, round(x$Proportion), x$Proportion); x})()
    birth_reg[, Proportion := as.numeric(Proportion)]
...
renderGirafe({
 ...
 # scale_fill_gradient(na.value = "white", low = "#B12A90FF", high="#0D0887FF", breaks=seq(0,120, by=20))+
  labs(x="", y="",fill="Percent (%)")+
...
school_att_county <- fread("data/table_2/table_2_12C_School_attendance_by_county.csv") |>
      as.data.table() |>
      subset(!(Category %in% "Gender Parity Index"))
...
school_att_county <- school_att_county[, cat := fifelse(Proportion < 52, "<52",
                      fifelse(Proportion > 51 & Proportion < 76, "51-75",
...
school_attendance <- fread("data/table_2/table_2_12_School_attendance_by_background_xtics.csv") |> (\(x) {x$Proportion <- fifelse(x$Proportion > 0.5, round(x$Proportion), x$Proportion); x})()
...
RTA <- fread("data/table_2/table_2_17_RTA_by_background_xtics.csv")
RTA$Proportion <- gsub(",","", RTA$Proportion)
RTA$Proportion <- as.numeric(RTA$Proportion)
...
RTA_county <- RTA_county[, Proportion := as.numeric(Proportion)]
breaks<- classIntervals(RTA_county$Proportion, n=5, style="quantile", na.rm=T)

# RTA_county <- RTA_county |>
#   mutate(cat=fifelse(Proportion<33, "<33", fifelse(Proportion>32 & Proportion<85, "33-84", fifelse(Proportion>84 & Proportion <211, "85-210",
#                                                                                                 fifelse(Proportion>210 &Proportion<1062, "211-1061", ">1062")))))

RTA_county <- RTA_county[, cat := fifelse(Proportion < 33, "<33",
                      fifelse(Proportion > 32 & Proportion < 85, "33-84",
                      fifelse(Proportion > 84 & Proportion < 211, "85-210",
                      fifelse(Proportion > 210 & Proportion < 1062, "211-1061", ">1062"))))]
...
# covid_county1 <- fread("data/table_2/table_2_19_1C_covid_by_county.csv")
...
covid_county3 <- covid_county3[, cat := fifelse(Proportion < 9.6, "<9.6",
                      fifelse(Proportion > 9.5 & Proportion < 18.7, "9.6-18.6",
                      fifelse(Proportion > 18.6 & Proportion < 30.1, "18.7-30.0",
                      fifelse(Proportion > 30.0 & Proportion < 8.0, "30.1-47.9", ">48.0"))))]
...
covid$Value<- fct_relevel(covid$Value, "Lowest", "Second", "Middle", "Fourth", "Highest")
...
renderGirafe({if(input$covid%in%"Residence"){
  ...
  facet_wrap(Participants~Category, nrow=1)+
     scale_fill_manual(values= brewer_pal(palette = "Greens")(5))+
 # scale_fill_gradient(na.value = "white", low = "#F0F921FF", high="#E16462FF")+
  labs(x="", y="",fill="Percent (%)")+
  theme(text=element_text(size=14))
...
cash_transfer_county <- cash_transfer_county[, cat := fifelse(Proportion < 0.7, "<0.7",
fifelse(Proportion > 0.6 & Proportion < 2.6, "0.7-2.5",
fifelse(Proportion > 2.5 & Proportion < 8.1, "2.6-8.0",
fifelse(Proportion > 8.0 & Proportion < 20.0, "8.1-19.9", ">19.9"))))]
...
# cash_transfer_county1 <- cash_transfer_county1 |>
#   mutate(cat=fifelse(Proportion<0.7, "<0.7", fifelse(Proportion>0.6 & Proportion<2.6, "0.7-2.5", fifelse(Proportion>2.5 & Proportion <8.1, "2.6-8.0",
#                                                                                                 fifelse(Proportion>8.0&Proportion<20.0, "8.1-19.9", ">19.9")))))
...
respondentsdata <- fread("data/table_2/table2_respondentsdata.csv") |>
      as.data.table() |>
      (\(x) {x$Weighted_percent <- fifelse(x$Weighted_percent > 0.5, round(x$Weighted_percent), x$Weighted_percent); x})()
...
background_characteristics_map <- fread("data/table_2/table_2C_backgroundxtics_by_county.csv")
...
background_characteristics_map <- background_characteristics_map[, cat := fifelse(weighted_perc<0.8, "<0.8",
fifelse(weighted_perc>0.7 & weighted_perc<1.3, "0.8-1.2", fifelse(weighted_perc>1.2 & weighted_perc <2.1, "1.3-2.0",                      fifelse(weighted_perc>2.0&weighted_perc<2.9, "2.1-2.8", ">2.8"))))]

background_characteristics_map <- full_join(county2, background_characteristics_map, by="County")
...
# mutate(cat=fifelse(weighted_perc<0.8, "<0.8",
# fifelse(weighted_perc>0.7 & weighted_perc<1.3, "0.8-1.2", fifelse(weighted_perc>1.2 & weighted_perc <2.1, "1.3-2.0",                      fifelse(weighted_perc>2.0&weighted_perc<2.9, "2.1-2.8", ">2.8")))))
...
renderGirafe({
  ...
  #geom_text(aes(label=paste0(Weighted_percent,"%")), hjust = 0.7, size=4, color="black")+
...
```{r, include=F}
#occupation_county <- read.csv("data/table_3/table3_7C_occupation_by_county.csv")
...
occupation_county <- fread("data/table_3/table3_7C_occupation_by_county_2.csv")

occupation_county <- occupation_county[, cat := fifelse(Proportion < 1.9, "<1.9",
fifelse(Proportion > 1.8 & Proportion < 5.5, "1.9-5.4",
fifelse(Proportion > 5.4 & Proportion < 9.5, "5.5-9.4",
fifelse(Proportion > 9.4 & Proportion < 18.5, "9.5-18.4", ">18.4"))))]

occupation_county3 <- full_join(county2,occupation_county, by=c("County"="County"))
...
# |>
#   mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
occupation <- occupation[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)]  
...
# |>
#   mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
employment <- employment[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)]  
...
#   mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
migration <- migration[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)]  
...
#|>
  # mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
water_collector <- water_collector[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)]  
...
water_collector_county1 <- water_collector_county[, cat := fifelse(Proportion < 3.1, "<3.1",
fifelse(Proportion > 3.0 & Proportion < 7.1, "3.1-7.0",
fifelse(Proportion > 7.0 & Proportion < 17.5, "7.1-17.4",
fifelse(Proportion > 17.4 & Proportion < 62.0, "17.5-61.9", ">61.9"))))]
...
# water_collector_county1 <- water_collector_county1 |>
#   mutate(cat=fifelse(Proportion<3.1, "<3.1", fifelse(Proportion>3.0 & Proportion<7.1, "3.1-7.0", fifelse(Proportion>7.0 & Proportion <17.5, "7.1-17.4",
#                                                                                                 fifelse(Proportion>17.4&Proportion<62.0, "17.5-61.9", ">61.9")))))
...
#|>
#  mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
water_treat <- water_treat[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)]  
...
water_treat_county <- water_treat_county[, cat := fifelse(Proportion < 0.3, "<0.3",
fifelse(Proportion > 0.2 & Proportion < 3.0, "0.3-2.9",
fifelse(Proportion > 2.9 & Proportion < 25.3, "3.0-25.2", ">25.2")))]
...
# water_treat_county1 <- water_treat_county1 |>
#   mutate(cat=fifelse(Proportion<0.3, "<0.3", fifelse(Proportion>0.2 & Proportion<3.0, "0.3-2.9", fifelse(Proportion>2.9 & Proportion <25.3, "3.0-25.2",
#                                                                                                  ">25.2"))))
...
#|>
#mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
excreta_mgmnt <- excreta_mgmnt[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)]  
...
excreta_mgmnt_county <- excreta_mgmnt_county[, cat := fifelse(Proportion < 1, "<1.0",
fifelse(Proportion > 0.9 & Proportion < 10.7, "0.9-10.6",
fifelse(Proportion > 10.6 & Proportion < 47.8, "10.7-47.7", ">47.7")))]
...
#|>
  #mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
child_stool_disp <- child_stool_disp[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)]  
...
child_stool_disp_county1  <- child_stool_disp_county [, cat := fifelse(Proportion < 2, "<2",
fifelse(Proportion > 1.9 & Proportion < 10.4, "2.0-10.3",
fifelse(Proportion > 10.3 & Proportion < 57.2, "10.4-57.1", ">57.1")))]
...
# child_stool_disp_county1 <- child_stool_disp_county1 |>
#   mutate(cat=fifelse(Proportion<2, "<2", fifelse(Proportion>1.9 & Proportion<10.4, "2.0-10.3", fifelse(Proportion>10.3 & Proportion <57.2, "10.4-57.1",
#                                                                                                  ">57.1"))))
...
handwashing <- handwashing[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)]  
...
handwashing_county1  <- handwashing_county [, cat := fifelse(Proportion < 13.1, "<13.1",
fifelse(Proportion > 13.0 & Proportion < 39.8, "13.1-39.7",
fifelse(Proportion > 39.7 & Proportion < 63.8, "39.8-63.7",">63.7")))]
...
# handwashing_county1 <- handwashing_county1 |>
#   mutate(cat=fifelse(Proportion<13.1, "<13.1", fifelse(Proportion>13.0 & Proportion<39.8, "13.1-39.7", fifelse(Proportion>39.7 & Proportion <63.8, "39.8-63.7",
#                                                                                                  ">63.7"))))
...
marriage_data1 <- marriage_data [, cat := fifelse(Value < 16.1, "<16.1",
fifelse(Value > 16.0 & Value < 20.1, "16.1-20.0",
fifelse(Value > 20.0 & Value < 40.1, "20.1-40.0", ">40.0")))]
...
# marriage_data1 <- marriage_data1 |>
#   mutate(cat=fifelse(Value<16.1, "<16.1", fifelse(Value>16.0 & Value<20.1, "16.1-20.0", fifelse(Value>20.0 & Value <40.1, "20.1-40.0",
#                                                                                                  ">40.0"))))
...
fertility_rate <- fertility_rate[Variable %in%"TFR"]
...
TFRbycounty.2022 <- fread("data/table_5/Table 5.2_C  Fertility by county.2022_bind.csv")

TFRbycounty.2022_1 <- TFRbycounty.2022 [, cat := fifelse(Proportion < 3.2, "<3.2",
fifelse(Proportion > 3.1 & Proportion < 3.6, "3.2-3.5",
fifelse(Proportion > 3.5 & Proportion < 4.4, "3.6-4.3", ">4.3")))]
...
# TFRbycounty.2022_1 <- TFRbycounty.2022_1 |>
#   mutate(cat=fifelse(Proportion<3.2, "<3.2", fifelse(Proportion>3.1 & Proportion<3.6, "3.2-3.5", fifelse(Proportion>3.5 & Proportion <4.4, "3.6-4.3",
#                                                                                                  ">4.3"))))
...
median_age <- median_age[category%in%"Women aged 25-49"]
...
#   mutate(proportion_category=fifelse(proportion<20,"<20",">=20"))|>
...
median_age_county1 <- median_age_county[, cat := fifelse(proportion < 19.9, "<19.9",
fifelse(proportion > 19.9 & proportion < 20.8, "19.9-20.7",
fifelse(proportion > 20.7 & proportion < 21.7, "20.8-21.1","21.2-22.3")))]
...
# median_age_county1 <- median_age_county1 |>
#   mutate(cat=fifelse(proportion<19.9, "<19.9", fifelse(proportion>19.9 & proportion<20.8, "19.9-20.7", fifelse(proportion>20.7 & proportion <21.2, "20.8-21.1",
#                                                                                                  "21.2-22.3"))))
...
teenpregnancybycountyvisual <- teenpregnancybycountyvisual[,proportion_category := fifelse(Percent<10, "<10%",
fifelse(Percent>9.9 & Percent < 21, "10-20%", ">20%"))]
...
teenpregnancybycountyvisual <- teenpregnancybycountyvisual[, cat := fifelse(Percent < 1.5, "<1.5",
fifelse(Percent > 1.4 & Percent < 5.4, "1.5-5.3",
fifelse(Percent > 5.3 & Percent < 12.7, "5.4-12.6","12.7-50.1")))]
...
# teenpregnancybycountyvisual1 <- teenpregnancybycountyvisual1 |>
#   mutate(cat=fifelse(Percent<1.5, "<1.5", fifelse(Percent>1.4 & Percent<5.4, "1.5-5.3", fifelse(Percent>5.3 & Percent <12.7, "5.4-12.6",
#                                                                                                  "12.7-50.1"))))
...
renderGirafe({
  ...
  geom_sf_interactive(aes(fill=cat, tooltip=paste0(Name,": ",Percent)),color="grey80", size=0.0)+
    ...
    scale_fill_manual(values=brewer_pal(palette = "Reds")(5))+
    #scale_fill_brewer(palette = "YlOrBr")+  
   # scale_fill_manual(values = c("#f0f921ff","#fca636ff","#e16462ff"))+
    #scale_fill_gradient(low = "#fee6ce", high = "#7f2704")+
    labs(x="", y="",fill="Percent (%)")+
    theme(text=element_text(size=14, face="bold")) 
...
# |>
#   mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
knowledge_fertile <- knowledge_fertile[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
reasons_discontinuation <- reasons_discontinuation[variable%in%"All methods"]
reasons_discontinuation <- reasons_discontinuation[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
```
...
need_demand <- need_demand[need%in%"Unmet need"]
...
decision_making1a <- decision_making1a[variable%in%"General"]
decision_making1a <- decision_making1a[, proportion := fifelse(proportion>0.5, round(proportion), proportion)] 
...
fp_messages <- fp_messages[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
```
...
fp_messages_county <- fp_messages_county[, proportion_category := fifelse(Proportion < 20, "20%", 
fifelse(Proportion > 19.9 & Proportion < 41, "20%-40%",
fifelse(Proportion > 40.9 & Proportion < 61, "41%-60%",
fifelse(Proportion>60.9&Proportion<81,"61%-80%",">80%"))))]
...
fp_messages_county1 <- fp_messages_county[!Category%in%"Community meeting/events"]

fp_messages_county1 <- full_join(county, fp_messages_county1, by=c("Name"="County"))
...
#   mutate(Variable=fifelse(Characteristic%in%c("Lowest","Second","Middle","Fourth","Highest"),"Wealth quintile",Variable))
...
early_childhood_mortality_county <-fread("data/DHS County tables/Table_8.3C_grp_County Early Childhood Mortality.csv") #|>
  #mutate(proportion_category=fifelse(proportion<20,"<20", fifelse(proportion>19&proportion<41,"20-40",">40")))
 ...
 if(input$early_childhood_mortality!="County"){
  ...
  mort_map2x<-girafe(ggobj = mort_map2, 
                   options = list(
                     ...
                     opts_zoom(max = 4)), width=15, height=5)
  print(mort_map2x)}
}) %>% bindCache(input$early_childhood_mortality) #,width = 1500, height=800) 
```
...
perinatal_mortality <- perinatal_mortality[!(category %in% "Pregnancies (28 weeks)") & category %in% "Perinatal mortality rate", ]
perinatal_mortality <- perinatal_mortality[characteristic %in% c("No education", "Primary", "Secondary", "More than secondary"), variable := "Education"]
perinatal_mortality <- perinatal_mortality[proportion > 0.5, proportion := round(proportion)]


# perinatal_mortality <- perinatal_mortality |> mutate(variable=fifelse(characteristic%in%c("No education","Primary","Secondary","More than secondary"),"Education",variable))|>
#  mutate(proportion=fifelse(proportion>0.5, round(proportion), proportion))
...
perinatal_mortality_county <- perinatal_mortality_county[category %in% "Perinatal mortality rate"]
...
renderGirafe({
  ...
  mort_map2x<-girafe(ggobj = mort_map2,
                   options = list(
                     ...
                     opts_zoom(max = 4)), width=15, height=5)
  print(mort_map2x)}
}) %>% bindCache(input$perinatal_mortality) #,width = 1500, height=800)
```
...
antenatal_care1 <- antenatal_care1[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
anc_number_county <- anc_number_county[, proportion_category := ifelse(Proportion < 20, "<20", 
                                   ifelse(Proportion > 19 & Proportion < 40, "20-39", "40-78"))]
...
}) %>% bindCache(input$antenatal_care_visits) #,width = 1500, height=800)
```
...
c_section <- c_section[, proportion_category := ifelse(percentage_delivered_by_cs < 10, "<10", ifelse(percentage_delivered_by_cs > 9 & percentage_delivered_by_cs < 21, "10-20", ">20"))]
...
#   mutate(County=recode(County,"Elgeyo-Marakwet"="Elgeyo Marakwet", "Murang'a"="Muranga", "Tharaka-Nithi"="Tharaka Nithi"))|>
#   mutate(proportion_category=fifelse(proportion<80,"<80",">80"))
...
mens_involvement <- mens_involvement[!Characteristic%in%"<20"]
...
vaccination_data  <- vaccination_data[, Vaccines1 := ifelse(Vaccines %in% c("Rotavirus1", "Rotavirus2", "Pneumococcal1", "Pneumococcal2", "Pneumococcal3","OPV0", "OPV1", "OPV2", "OPV3", "MR1", "MR2", "IPV", "BCG", "DPT1", "DPT2", "DPT3"), "Vaccines", "Summary")]
...
<!--   mutate(names=fifelse(sought_treatment%in%NA, paste0(Name," No value"), paste0(Name,", ", sought_treatment,"%"))) -->
...
treat_source <- treat_source[, `Source of Treatment` := ifelse(`Source of Treatment` %in% c("Private medical sector (NGO)", "Private medical sector (non-NGO)", "Other private sector"), "Private sector", `Source of Treatment`)]
treat_source <- treat_source[, Proportion := sum(Proportion, na.rm = TRUE), by = .(`Source of Treatment`, Condition, Category)]
treat_source <- treat_source[, .(`Source of Treatment`, Condition, Category, Proportion)]
treat_source <- unique(treat_source)
treat_source <- treat_source[, Proportion := ifelse(Proportion > 0.5, round(Proportion), Proportion)]
...
}) %>% bindCache(input$nutr)#,width = 1500, height=800)
...
# |>
  # mutate(Proportion=fifelse(Proportion<0.5, Proportion, round(Proportion)))
micronutrient <- micronutrient[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)]
...
iycf <- iycf[, Proportion := ifelse(Proportion < 0.5, Proportion, round(Proportion))]
iycf <- iycf[, Indicator := gsub("Percentage", "%", Indicator)]
  #mutate(Proportion=fifelse(Proportion<0.5, Proportion, round(Proportion))) |>
...
nutrition_women <- nutrition_women[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
# |>
#   mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
mdd <- mdd[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
mdd_county1 <- full_join(county, mdd_county, by=c("Name"="County"))#|>
  #mutate(proportion_category=fifelse(Proportion<15,"<15%", fifelse(Proportion>14&Proportion<31,"15%-30%",">30%")))
...
# |>
#   mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
iron_women <- iron_women[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
itn_possess <- fread("data/table_12/table12_1_ITN_possession_by_background_xtics.csv") #|>
 # mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
itn_possess <- itn_possess[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
itn_source_county <- itn_source_county[Category %in% c("Mass distribution campaign","Shop/market","ANC visit","Immunization visit")]
...
itn_access <- itn_access[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
itn_use <- itn_use[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
# |>
#   mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
existing_itn_use <- existing_itn_use[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
existing_itn_use_county <- existing_itn_use_county[, epi_zone := fifelse(
  Names %in% c("Kisii", "Nyamira", "West Pokot", "Trans Nzoia", "Uasin Gishu", "Nandi", "Narok", "Kericho", "Bomet", "Elgeyo Marakwet", "Turkana", "Kajiado"),
  "Highland Epidemic Prone",
  fifelse(Names %in% c("Siaya", "Kisumu", "Migori", "Homa Bay", "Kakamega", "Vihiga", "Bungoma", "Busia", "Bungoma", "Kakamega"),"Lake Stable Endemic",
   fifelse(Names %in% c("Mombasa", "Kwale", "Kilifi", "Lamu", "Taita Taveta","Tana River"),"Coast Stable Endemic",
  fifelse(Names %in% c("Marsabit", "Isiolo", "Meru", "Tharaka Nithi", "Embu", "Kitui", "Garissa", "Wajir", "Mandera", "Samburu", "Baringo", "Elgeyo Marakwet", "West Pokot", "Makueni"),"Seasonal Low Transmission","Low Risk"))))]


existing_itn_use_county1 <- full_join(county, existing_itn_use_county, by=c("Name"="County"))  # |> mutate(proportion_category=fifelse(Proportion<30,"<30%", fifelse(Proportion>30 & Proportion<61,"30%-60%",">60%")))|>
...
children_itn_use <- children_itn_use[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
children_itn_use_county <- children_itn_use_county[, names1 := paste0(County, ": ", Proportion, "%")]
children_itn_use_county <- children_itn_use_county[, epi_zone := fifelse(
  County %in% c("Kisii", "Nyamira", "West Pokot", "Trans Nzoia", "Uasin Gishu", "Nandi", "Narok", "Kericho", "Bomet", "Elgeyo Marakwet", "Turkana", "Kajiado"),
  "Highland Epidemic Prone",
  fifelse(County %in% c("Siaya", "Kisumu", "Migori", "Homa Bay", "Kakamega", "Vihiga", "Bungoma", "Busia", "Bungoma", "Kakamega"),"Lake Stable Endemic",
   fifelse(County %in% c("Mombasa", "Kwale", "Kilifi", "Lamu", "Taita Taveta","Tana River"),"Coast Stable Endemic",
  fifelse(County %in% c("Marsabit", "Isiolo", "Meru", "Tharaka Nithi", "Embu", "Kitui", "Garissa", "Wajir", "Mandera", "Samburu", "Baringo", "Elgeyo Marakwet", "West Pokot", "Makueni"),"Seasonal Low Transmission","Low Risk"))))]
...
#|>
#   mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
women_itn_use <- women_itn_use[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
#   filter(Reason%in%c("Extra net/saving for later","No mosquitoes/malaria","User didn't sleep in household","Too hot"))|>
#   mutate(Proportion=fifelse(County%in%"Kitui"&Reason%in%"Extra net/saving for later",21.1, fifelse(County%in%"Kitui"&Reason%in%"No mosquitoes/malaria",28.2, fifelse(County%in%"Kitui"&Reason%in%"User didn't sleep in household",9.2, fifelse(County%in%"Kitui"&Reason%in%"Too hot",24.2,Proportion)))))

reason_itn_not_used_county <- fread("data/table_12/table12_8C_grp_reason_itn_not_used_by_county_2.csv")

reason_itn_not_used_county <- reason_itn_not_used_county[, names1 := paste0(Names, ": ", Proportion, "%")]
reason_itn_not_used_county <- reason_itn_not_used_county[, epi_zone := fifelse(
  Names %in% c("Kisii", "Nyamira", "West Pokot", "Trans Nzoia", "Uasin Gishu", "Nandi", "Narok", "Kericho", "Bomet", "Elgeyo Marakwet", "Turkana", "Kajiado"),
  "Highland Epidemic Prone",
  fifelse(Names %in% c("Siaya", "Kisumu", "Migori", "Homa Bay", "Kakamega", "Vihiga", "Bungoma", "Busia", "Bungoma", "Kakamega"),"Lake Stable Endemic",
   fifelse(Names %in% c("Mombasa", "Kwale", "Kilifi", "Lamu", "Taita Taveta","Tana River"),"Coast Stable Endemic",
...
iptp <- iptp[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
iptp_county <- iptp_county[, names1 := paste0(County, ": ", Proportion, "%")]
iptp_county <- iptp_county[, epi_zone := fifelse(
  County %in% c("Kisii", "Nyamira", "West Pokot", "Trans Nzoia", "Uasin Gishu", "Nandi", "Narok", "Kericho", "Bomet", "Elgeyo Marakwet", "Turkana", "Kajiado"),
  "Highland Epidemic Prone",
  fifelse(County %in% c("Siaya", "Kisumu", "Migori", "Homa Bay", "Kakamega", "Vihiga", "Bungoma", "Busia", "Bungoma", "Kakamega"),"Lake Stable Endemic",
   fifelse(County %in% c("Mombasa", "Kwale", "Kilifi", "Lamu", "Taita Taveta","Tana River"),"Coast Stable Endemic",
...
# |>
#   mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
mal_fever <- mal_fever[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
mal_fever_county <- mal_fever_county[, names1 := paste0(County, ": ", Proportion, "%")]
mal_fever_county <- mal_fever_county[, epi_zone := fifelse(
  County %in% c("Kisii", "Nyamira", "West Pokot", "Trans Nzoia", "Uasin Gishu", "Nandi", "Narok", "Kericho", "Bomet", "Elgeyo Marakwet", "Turkana", "Kajiado"),
  "Highland Epidemic Prone",
  fifelse(County %in% c("Siaya", "Kisumu", "Migori", "Homa Bay", "Kakamega", "Vihiga", "Bungoma", "Busia", "Bungoma", "Kakamega"),"Lake Stable Endemic",
   fifelse(County %in% c("Mombasa", "Kwale", "Kilifi", "Lamu", "Taita Taveta","Tana River"),"Coast Stable Endemic",
...
mal_treat_source <- mal_treat_source[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
hiv_meds_kap <- hiv_meds_kap[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
hivc1 <- fread("data/table_23/table23_hiv_count_data_2.csv")

hivc1 <- full_join(county, hivc1,by=c("Name"="County"))
...
# |>
#   mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
bp <- bp[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
bp_county1 <- full_join(county, bp_county, by=c("Name"="County"))|>
  #mutate(proportion_category=fifelse(Proportion<10,"<10%", fifelse(Proportion>10 & Proportion<21,"10%-20%",">20%")))|>
  mutate(names1=paste0(Name,": ",Proportion,"%"))
...
```{r}
# 
# renderPlot({if(input$bld_sugar%in%c("Age","Residence", "Education","Wealth quintile")){ggplot(bld_sugar[bld_sugar$`Background characteristics`%in%input$bld_sugar,],aes(x=Value, y=Proportion,fill=Category))+
#   geom_bar(position="dodge", stat="identity")+
#   theme_bw()+
#   facet_wrap(.~Participants)+
#   labs( y="Proportion (%)", title="", x="", fill="")+
...
depression <- depression[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)]
...
depression_county <- depression_county[, Category := ifelse(Category == "Diagnosed with Depression", "Diagnosed with Depression and/or Anxiety", Category)]
...
# |>
#   mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
arthritis <- arthritis[, Proportion := ifelse(Proportion > 0.5, round(Proportion), Proportion)]
...
physical_activity <- physical_activity[, `Activity time` := round(`Activity time`)]
...
physical_activity_county1 <- full_join(county, physical_activity_county, by=c("Name"="County"))|>
  #mutate(time_category=fifelse(`Activity time`<200,"<200 minutes", fifelse(`Activity time`>200 & `Activity time`<401,"200 minutes-400 minutes",fifelse(`Activity time`>400&`Activity time`<601,"401-600 minutes",">600 minutes"))))|>
...
# |>
#   mutate(Proportion=fifelse(Proportion>0.5,round(Proportion), Proportion))
breast_cancer <- breast_cancer[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
breast_cancer_county1 <- full_join(county, breast_cancer_county, by=c("Name"="County"))|>
  #mutate(proportion_category=fifelse(Proportion<20,"<20%", fifelse(Proportion>20 & Proportion<41,"20%-40%",">40%")))|>
  mutate(names1=paste0(Name,": ",Proportion,"%"))
...
# |>
#   mutate(Proportion=fifelse(Proportion>0.5, round(Proportion), Proportion))
cervival_cancer <- cervical_cancer[, Proportion := fifelse(Proportion>0.5, round(Proportion), Proportion)] 
...
T15.1 <- T15.1[, value := ifelse(value > 0.5, round(value), value)]
...
T15.2.2 <- T15.2.2[, value := as.numeric(value)]
T15.2.2 <- T15.2.2[, value := ifelse(value > 0.5, round(value), value)]
...
Agricultural_land_ownsership <- Agricultural_land_ownsership[, value := ifelse(value > 0.5, round(value), value)]
...
agricultural_land_ownsership_county <- agricultural_land_ownsership_county[, variable := ifelse(variable == "Does.not.have.a.title.deed", "Does not have \na title deed", variable)]


Agricultural_land_ownsership$Category <- fct_relevel(Agricultural_land_ownsership$Category, "No education", "Primary", "Secondary", "More than secondary","Lowest", "Second", "Middle", "Fourth", "Highest")
...
non_agricultural_land_ownsership <- non_agricultural_land_ownsership[, value := ifelse(value > 0.5, round(value), value)]
...
non_agricultural_land_ownsership_county <- non_agricultural_land_ownsership_county[, value := abs(value)]
non_agricultural_land_ownsership_county <- non_agricultural_land_ownsership_county[!is.na(variable)]
...
T15.6.1 <- T15.6.1[, value := ifelse(value > 0.5, round(value), value)]
...
decision_making <- decision_making[, value := ifelse(value > 0.5, round(value), value)]
...
T15.10 <- T15.10[, value := ifelse(value > 0.5, round(value), value)]
T15.10 <- T15.10[, Category := ifelse(Category == "Divorced/Separated/Widowed", "Divorced/\nSeparated/Widowed", 
ifelse(Category == "Married/Living together", "Married/\nLiving together", Category))]
...
T15.10C <- T15.10C[, value := abs(value)]
T15.10C <- T15.10C[, cat := ifelse(value < 40, "<40", 
                   ifelse(value > 39 & value < 60, "40-59", 
                          ifelse(value > 59 & value < 80, "60-79", ">79")))]
...
renderGirafe({if(input$safer_sex!="County"){
 ...
 theme_void()+
      scale_fill_manual(values=brewer_pal(palette = "Greens")(5))+
      #scale_fill_gradient(low="#ffffcc", high="#bd0026", na.value = "white")+
 labs(fill="%") +
  theme(text=element_text(size=14),axis.title=element_text(size=14,face="bold"))
...
physical_violence <- physical_violence[, value := ifelse(value > 0.5, round(value), value)]
...
pers_physical_violence <- pers_physical_violence[, value := ifelse(value > 0.5, round(value), value)]
...
# |>
#   mutate(value=fifelse(value>0.5, round(value), value))
violence_preg <- violence_preg[, value := ifelse(value > 0.5, round(value), value)]
...
# |>
#   mutate(value=fifelse(value>0.5, round(value), value))
sexual_violence <- sexual_violence[, value := ifelse(value > 0.5, round(value), value)]
...
# |>
#   mutate(value=fifelse(value>0.5, round(value), value))
sexual_violence1 <- sexual_violence1[, value := ifelse(value > 0.5, round(value), value)]
...
controlling <- controlling[, value := ifelse(value > 0.5, round(value), value)]
```
...
# |>
#   mutate(value=fifelse(value>0.5, round(value), value))
violence_spouse <- violence_spouse[, value := ifelse(value > 0.5, round(value), value)]
...
# |>
#   mutate(value=fifelse(value>0.5, round(value), value))
violence_women <- violence_women[, value := ifelse(value > 0.5, round(value), value)]


violence_women$Category <- fct_relevel(violence_women$Category, "No education", "Primary", "Secondary", "More than secondary","Lowest", "Second", "Middle", "Fourth", "Highest")
...
renderGirafe({if(!(input$violence_women %in% c("County","Marital status"))){
...
T17.17 <- T17.17[, value := ifelse(value > 0.5, round(value), value)]
#mutate(value=fifelse(value>0.5, round(value), value))
...
T17.17C <- T17.17C[, value := as.numeric(value)]
...
#   geom_col(position="dodge") + geom_text(aes(label=paste0(value,"%")), position=position_dodge(0.7), vjust=-0.5)+
#   theme_bw() +scale_fill_brewer(palette = "Set1")+
#   labs( x = "", y="Percent (%)", fill="")+theme(text=element_text(size=20, face="bold"))}, width=1500, height=700)
...
#    mutate(Prop=fifelse(P%in%"Proportion",paste0("Proportion", " (", year,")"), P)) |>
#   select(-P)
...
setDT(comparison_data1a)[, indicator := recode(Category, "Households with >=1 ITN"="Malaria", "Households with >=1 ITN\nper 2 people"="Malaria", "Stunting"="Nutrition", "Underweight"="Nutrition", "Wasting"="Nutrition", "BCG"="Child vaccination", "DPT1"="Child vaccination", "DPT2"="Child vaccination", "DPT3"="Child vaccination", "FULL_VACCINATION"="Child vaccination", "OPV"="Child vaccination", "OPV1"="Child vaccination", "OPV2"="Child vaccination", 
"OPV3"="Child vaccination")]
...
renderGirafe({
  trends_data1 <-trends_data[indicator%in%input$trends]
  trends_data1 <- trends_data1[, Years := as.numeric(Years)]
...
renderGirafe({
  comp_data1 <- comparison_data2[comparison_data2$Category%in%input$ind_comp,]
#comparison_data2 |>filter(Category%in%input$ind_comp) 
...
combined_data2 <- combined_data[combined_data$indicator1 %in% input$comp_indicator2, ] |> rename(indicator2 = indicator1, values2 = values) |> select(-indicator)

combined_data1 <- left_join(combined_data1, combined_data2)
...
combined_data1 <- combined_data[combined_data$indicator1 %in% input$comp_indicator1, ] |> select(-indicator)
combined_data1 <- left_join(county, combined_data1, by = c("Name" = "County"))
